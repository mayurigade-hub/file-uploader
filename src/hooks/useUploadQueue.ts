...
export const useUploadQueue = () => {
    ...
    const [globalCommand, setGlobalCommand] = useState<{ type: 'pause' | 'resume' | 'none'; ts: number }>({
        type: 'none',
        ts: 0
    });
    ...
    const pauseAll = useCallback(() => {
        setGlobalCommand({ type: 'pause', ts: Date.now() });
    }, []);

    const resumeAll = useCallback(() => {
        setGlobalCommand({ type: 'resume', ts: Date.now() });
    }, []);
